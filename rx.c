...
#define ASCII\
	...
	X(TK_HT,"TK_HT",L_NOP) X(TK_LF,"TK_LF",L_LF) X(TK_VT,"TK_VT",L_NOP)\
...
#define LEX_CLR()({lex.line=1;lex.column=0;lex.current=0;})
...
#define ISLBL(L)({load_or_repl[lex.current]==&&L})
#define NOTLBL(L)({load_or_repl[lex.current]!=&&L})
#define ISC(C)({lex.current==C})
#define NOTC(C)({lex.current!=C})
#define LEX_CURTK()({tokens.data[tokens.length];})
/*****************************************************************************/
/* Lexer Operations */
/*****************************************************************************/
/*****************************************************************************/
...
{\
	...
	L_LEX:\
		printf("c:%c\n",lex.current);\
		goto *token_ops[lex.current];\
	L_LF:\
		lex.line++;\
		lex.column=0;\
		goto *interpret_or_repl[lex.repl];\
	L_LETTER:goto L_INTERPRET;\
	L_DIGIT:goto L_INTERPRET;\
	L_ID:goto L_INTERPRET;\
	L_NUM:goto L_INTERPRET;\
	L_STR:goto L_INTERPRET;\
	L_ASCII:goto L_INTERPRET;\
	L_NOP:goto L_INTERPRET;\
}
#define quit()\
{\
	fclose(lex.stream);\
...
void main(int nargs,char** args)
{
	...
	#undef X
	#define X(A,B,C) &&C,
	static void* token_ops[]={ASCII TOKEN_TYPES};
